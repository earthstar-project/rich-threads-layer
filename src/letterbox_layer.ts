import {
  ...
  ValidationError,
  WriteResult,
} from "https://cdn.skypack.dev/earthstar?dts";
import {
  deleteEdgeSync,
  findEdgesSync,
  GraphEdgeContent,
  writeEdgeSync,
} from "https://cdn.skypack.dev/earthstar-graph-db?dts";
...
export default class LetterboxLayer {
  ...
  _docToThreadRoot(rootDoc: Document): ThreadRoot {
    const publishedTimestamp = this.getDocPublishedTimestamp(rootDoc);
  ...
  getDocPublishedTimestamp(doc: Document): number {
  ...
  _docToPost(postDoc: Document): Post {
    const publishedTimestamp = this.getDocPublishedTimestamp(postDoc);
    ...
    return [thread.root, ...thread.replies].map(({ doc }) => doc).some(
      (doc) => {
        return this.isUnread(
          thread.root.id,
          this.getDocPublishedTimestamp(doc),
        );
      },
    );
  }
